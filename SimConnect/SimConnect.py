...
class SimConnect:
    ...
    def handle_id_event(self, event):
        ...
        for listener in self.event_listeners:
            listener.handle_id_event(event.uEventID)
        ...
        if dwRequestID in self.Requests:
            ...
            if 'string' in rtype.lower():
                ...
                _request.outData = cast(
                    ObjData.dwData, POINTER(
                        c_double * len(_request.definitions))
                ).contents[0]
        else:
            LOGGER.warn("Event ID: %d Not Handled." % (dwRequestID))
        for listener in self.event_listeners:
            listener.handle_simobject_event(ObjData)
    ...
    def handle_exception_event(self, exc):
        ...
        for _reqin in self.Requests:
            ...
            if _request.LastID == _unsendid:
                ...
                definition = _request.definitions[0]
                LOGGER.warn("%s: in %s" % (exception, definition))
                for listener in self.event_listeners:
                    listener.handle_exception_event(exception, definition)
                return
    ...
    def _run(self):
        while self.quit == 0:
            try:
                self.dll.CallDispatch(
                    self.hSimConnect, self.my_dispatch_proc_rd, None)
                time.sleep(.002)
    ...
    def map_to_sim_event(self, name):
        ...
        err = self.dll.MapClientEventToSimEvent(
            self.hSimConnect, evnt.value, name)
        if self.IsHR(err, 0):
            return evnt
        else:
    ...
    def new_def_id(self):
        ...
        self.dll.DATA_DEFINITION_ID = Enum(
            self.dll.DATA_DEFINITION_ID.__name__, names)
    ...
    def new_request_id(self):
        ...
        self.dll.DATA_REQUEST_ID = Enum(
            self.dll.DATA_REQUEST_ID.__name__, names)
    ...
    def save_flight(
        ...
        hr = self.dll.FlightSave(self.hSimConnect, flt_path.encode(
        ), flt_title.encode(), flt_description.encode(), 0)
